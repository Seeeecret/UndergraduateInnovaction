...
def main():
    ...
    val_datset = datasets.ImageFolder(r'C:\\PHG\\Dataset\\PHG_Angle\\val\\pitch',transform)


    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers
    print('Using {} dataloader workers every process'.format(nw))
    trainLoader1 = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=False, num_workers=nw)
    trainLoader2 = torch.utils.data.DataLoader(dataset=train_dataset2, batch_size=batch_size, shuffle=False, num_workers=nw)
    validate_loader = torch.utils.data.DataLoader(dataset=val_datset, batch_size=batch_size, shuffle=False, num_workers=nw)
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            image1_edge = edge_detection(images1)
            image2_edge = edge_detection(images2)
            optimizer.zero_grad()
    ...
    with torch.no_grad():
        ...
        for val_data in val_bar:
            ...
            val_images_edge = edge_detection(val_images)
