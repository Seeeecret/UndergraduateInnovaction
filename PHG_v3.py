...
def main():
    ...
    # 定义数据集、loader数据集a
    train_dataset1 = datasets.ImageFolder(r'CPHGDatasetPHG_Angletrainpitch', transform)
    train_dataset2 = datasets.ImageFolder(r'CPHGDatasetPHG_Angletrainside', transform)
    val_dataset = datasets.ImageFolder(r'CPHGDatasetPHG_Anglevalpitch',transform)
    ...
    image1_list = []
    image2_list = []
    image1_list.append(data for data in enumerate(trainLoader1))
    image2_list.append(data for data in enumerate(trainLoader2))
    for epoch in range(epochs)
        running_loss = 0.0
        phgnet.train()
        if epoch % 10 < 5:
            train_bar = tqdm(trainLoader1)
        else:
            train_bar = tqdm(trainLoader2)
        for step, data in enumerate(train_bar)
            images1, labels = data
            if epoch % 10 < 5:
                images2 = image2_list[step]
            else:
                images2 =image1_list[step]
            
            image1_edge = color_detection(images1)
            image1_edge = edge_detection(image1_edge)
            image2_edge = color_detection(images2)
            image2_edge = edge_detection(image2_edge)
            optimizer.zero_grad()
    ...
    with torch.no_grad()
        ...
        for val_data in val_bar
            ...
            val_images_edge = edge_detection(val_images_color)
            outputs = net(val_images.to(device), val_images.to(device), val_images_edge.to(device), val_images_edge.to(device))
